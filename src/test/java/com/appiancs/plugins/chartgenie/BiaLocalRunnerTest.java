...
class BiaLocalRunnerTest {
  ...
  private static final Path PAYLOAD_PATH =
      Path.of("src/test/java/com/appiancs/plugins/chartgenie/bia-full-payload.json");

  @Test
  void generateBiaFullReport() throws Exception {
    ...
    Assumptions.assumeTrue(Files.exists(PAYLOAD_PATH),
        "BIA full payload not available in CI environment");
    ...
    assertNotNull(result, "Generated BIA report should not be null");
    assertTrue(result.length > 0, "Generated BIA report should not be empty");
  }
}
