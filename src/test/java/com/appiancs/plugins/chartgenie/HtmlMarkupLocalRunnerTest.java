...
class HtmlMarkupLocalRunnerTest {
  ...
  private static final Path PAYLOAD_PATH =
      Path.of("src/test/java/com/appiancs/plugins/chartgenie/html-markup-payload.json");

  @Test
  void generateHtmlMarkupReport() throws Exception {
    ...
    Assumptions.assumeTrue(Files.exists(PAYLOAD_PATH),
        "HTML markup payload not available in CI environment");
    ...
    assertNotNull(result, "Generated HTML markup report should not be null");
    assertTrue(result.length > 0, "Generated HTML markup report should not be empty");
  }
}
